...
def upload_archive(_args):
    filtered_file_paths = set(_args.file_paths)
    raw_inventory_dict = _get_raw_inventory(_args.vault)
    if _args.check_duplicates and raw_inventory_dict is not None:
        exist_archives = raw_inventory_dict.get("ArchiveList", [])
        exist_filenames = set(parse_description(record["ArchiveDescription"])["FileName"] for record in exist_archives)
        for fpath in _args.file_paths:
            filename = os.path.basename(fpath)
            if filename in exist_filenames:
                filtered_file_paths.remove(fpath)
                logger.warning(f"File {filename} already exists in the vault, skip.")

    upload_results = []
    for fn in filtered_file_paths:
        try:
    ...
    if raw_inventory_dict is not None:
        ...
        with open(
            os.path.join(get_meta_foler(), f"inventory_list_{_args.vault}.json"),
            "w",
            encoding="utf-8",
        ) as f:
...
def upload_one_file(vault_name, file_path, part_size, num_threads, upload_id=None):
    ...
    if part_size < 1 or part_size > 4096:
        raise ValueError("part-size must be more than 1 MB " "and less than 4096 MB")
    ...
    if file_size < 4096:
        ...
        response = glacier.upload_archive(vaultName=vault_name, archiveDescription=arc_desc, body=file_to_upload)
        ...
        result = {
            "CreationDate": nowtime.strftime("%Y-%m-%dT%H:%M:%SZ"),
            "ArchiveId": response["archiveId"],
            "ArchiveDescription": arc_desc,
            "Size": file_size,
            "SHA256TreeHash": response["checksum"],
        }
        return result
    ...
    if upload_id is None:
        ...
        while "Marker" in response:
            ...
            response = glacier.list_parts(vaultName=vault_name, uploadId=upload_id, marker=response["Marker"])
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
        ...
        done, not_done = concurrent.futures.wait(futures_list, return_when=concurrent.futures.FIRST_EXCEPTION)
        pbar.close()
        if len(not_done) > 0:
...
def upload_part(byte_pos, vault_name, upload_id, part_size, fileobj, file_size, fileblock):
    fileblock.acquire()
    ...
    range_header = "bytes {}-{}/{}".format(byte_pos, byte_pos + real_part_size - 1, file_size)
...
def submit_downloads(_args):
    ...
    archive_id_list = (
        _args.archive_id + archive_list.loc[archive_list.FileName.isin(_args.archive_name), "ArchiveId"].tolist()
    )
...
def parse_description(description):
    if description.startswith("<m>"):
        ...
        return pd.Series(
            [
                base64.b64decode(root.find("p").text).decode("utf-8"),
                pd.Timestamp(root.find("lm").text).tz_convert(tzlocal()),
            ],
...
def load_aborted_jobs():
    if os.path.exists(os.path.join(get_meta_foler(), f"job_aborted.json")):
        with open(os.path.join(get_meta_foler(), f"job_aborted.json"), "r", encoding="utf-8") as f:
            aborted_jobs = json.load(f)
        return aborted_jobs
    return []


def mark_jobs_as_aborted(_args):
    aborted_jobs = load_aborted_jobs()
    aborted_jobs.append(_args.job_ids)
    with open(os.path.join(get_meta_foler(), f"job_aborted.json"), "w", encoding="utf-8") as f:
        json.dump(aborted_jobs, f, indent=4)
...
def check_and_handle_jobs(_args):
    ...
    if os.path.exists(os.path.join(get_meta_foler(), f"watchdog_status_{_args.vault}.json")):
        ...
        with open(
            os.path.join(get_meta_foler(), f"watchdog_status_{_args.vault}.json"),
            "r",
            encoding="utf-8",
        ) as f:
            status = json.load(f)
        ...
        job_processed.update(status.get("Processed", dict()))
    try:
        while True:
            logger.info("Checking Jobs:")
            aborted_jobs = set(load_aborted_jobs())
            jobs = glacier.list_jobs(vaultName=_args.vault)
            if jobs["ResponseMetadata"]["HTTPStatusCode"] // 100 != 2:
            ...
            failed_flag = job_df.StatusCode == "Failed"
            aborted_flag = job_df.JobId.isin(aborted_jobs)
            need_process_flag = job_df.Completed & ~failed_flag & ~aborted_flag

            for jid, action, cdate in job_df.loc[need_process_flag, ["JobId", "Action", "CreationDate"]].values:
                if job_processed.get(jid, "") != cdate:
                    ...
                    if action == "InventoryRetrieval":
                        ...
                        download_job(
                            res,
                            _args.download_chunk_size * 1024**2,
                            pbar=_args.log_file == "",
                        )
            ...
            if flag.all():
                filtered_processed = {
                    k: v
                    for k, v in job_processed.items()
                    if pd.to_datetime(v) > pd.Timestamp.utcnow() - pd.Timedelta(days=3)
                }
                status["Processed"] = filtered_processed
                break

            remaining = job_df[~flag].copy()
            ...
            time.sleep((until - pd.Timestamp.utcnow()).total_seconds())
    except Exception:
        logger.error(traceback.format_exc())
    finally:
        status["Running"] = False
...
def list_inventory(_args):
    ...
    print(
        "\nVaultARN: ",
        raw_inventory["VaultARN"],
        "\nInventoryDate: ",
        raw_inventory["InventoryDate"],
        "\n",
    )
